import { ILesson, IChange } from '@yanshoof/types';
...
describe('Tests the settings module', () => {
  ...
  it('Tests a change that applies to the user', () => {
    expect(settings.isOwnChange(ENGLISH_CANCELLATION.day, ENGLISH_CANCELLATION.hour, ENGLISH_CANCELLATION)).toBe(true);
  });

  it('Tests a change that does not apply to the user', () => {
    expect(settings.isOwnChange(OTHERS_ENGLISH_CHANGE.day, OTHERS_ENGLISH_CHANGE.hour, OTHERS_ENGLISH_CHANGE)).toBe(
      false,
    );
  });
});
